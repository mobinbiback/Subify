...
### ترجمه و دوبلهٔ هوشمند زیرنویس یوتیوب به فارسی
...
**زیرنویس زمان‌بندی‌شدهٔ یوتیوب رو مستقیم، با ترجمه پیش‌دستانه و همگام‌سازی دقیق، به فارسی برگردون — یا حتی زنده دوبله کن.**
...
- 🌐 **ترجمهٔ لحظه‌ای زیرنویس یوتیوب** — مستقیم از کپشن‌های زمان‌بندی‌شدهٔ ویدیو، بدون نیاز به فایل زیرنویس جدا
- ⚡ **ترجمهٔ پیش‌دستانه (Pre-fetch)** با همگام‌سازی دقیق زمانی، برای تجربه‌ای بدون تأخیر
- 🎙️ **دوبلهٔ زندهٔ صوتی (Live Translate)** روی Gemini — تنظیم بلندی دوبله، میزان کات صدای اصلی (ducking)، سرعت پخش و انتخاب صدا
- 🔄 **حالت Chunked fallback** برای زمانی که مسیر Live در دسترس نیست
- 🧠 **چند ارائه‌دهندهٔ هوش مصنوعی**: Google Gemini · OpenRouter · OpenAI — با کلید API خودت
...
2. حالت دلخواه رو فعال کن: **ترجمهٔ متنی زیرنویس** یا **دوبلهٔ زندهٔ صوتی**
...
| مجوز | کاربرد |
|---|---|
| `storage` | ذخیرهٔ تنظیمات و کلیدهای API به‌صورت محلی |
| `tabCapture` / `offscreen` | ضبط صدای تب برای حالت دوبلهٔ زنده |
| `activeTab` / `tabs` | تشخیص و تعامل با تب یوتیوب فعال |
| `downloads` | ذخیرهٔ خروجی زیرنویس (SRT/VTT/TXT) |
