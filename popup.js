...
function renderState(s){
  ...
  browser.storage.local.get(["subify_subtitle_enabled"]).then(flags=>{
  ...
  $("status").innerHTML=err?`<span class="error">${escapeHtml(err)}</span>`:
    `<strong>وضعیت:</strong> زیرنویس از تایم‌کدهای واقعی خوانده می‌شود`;
}
...
async function init(){
  const [st,flags]=await Promise.all([browser.runtime.sendMessage({type:"popup-get-status"}),browser.storage.local.get("subify_subtitle_enabled")]);
...
(function initPopupTabsAndStyle(){
  ...
  function updateProviderUi(v,pd={}){
    ...
    if(isGemini){
      const val=pd.geminiSubtitleModel||pd.geminiModel||'gemini-3.1-flash-lite';
      select.innerHTML='';
      ...
      input.value=isOR?(pd.openRouterModel||'openrouter/free'):isOAI?(pd.openaiModel||'gpt-5.6-luna'):(pd.geminiSubtitleModel||'gemini-3.1-flash-lite');
...
$('subify-tab-subtitle')?.addEventListener('change',async e=>{await browser.storage.local.set({subify_subtitle_enabled:e.target.checked});const tab=await activeTab();if(tab?.id)try{await browser.tabs.sendMessage(tab.id,{type:e.target.checked?'subify-enable-subtitles':'subify-disable-subtitles'});}catch(_){} });
  ...
  $('popup-provider-model')?.addEventListener('change',async e=>{const {pd_settings={}}=await browser.storage.local.get('pd_settings');const provider=e.target.dataset.provider||$('popup-provider').value;const value=e.target.value.trim();const patch=provider==='openrouter'?{openRouterModel:value||'openrouter/free'}:provider==='openai'?{openaiModel:value||'gpt-5.6-luna'}:{geminiSubtitleModel:value||'gemini-3.1-flash-lite'};await browser.storage.local.set({pd_settings:{...pd_settings,...patch}});});
  $('popup-gemini-models')?.addEventListener('change',async e=>{const {pd_settings={}}=await browser.storage.local.get('pd_settings');const value=e.target.value;await browser.storage.local.set({pd_settings:{...pd_settings,geminiSubtitleModel:value}});});
  $('popup-gemini-model-manual')?.addEventListener('change',async e=>{const value=e.target.value.trim();if(!value)return;const {pd_settings={}}=await browser.storage.local.get('pd_settings');await browser.storage.local.set({pd_settings:{...pd_settings,geminiSubtitleModel:value}});});
